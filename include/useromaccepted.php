...
<div class="mt-5 mb-3 d-flex justify-content-between">
                        <h2 class="pull-left">Liste des ordres des missions approuvés</h2>
                    </div>
<?php 
require_once "config.php";
$nom = $_SESSION['nom'];                 
$sql = "SELECT * FROM ordred WHERE statut = 'Approuvé' AND nom ='$nom' ORDER BY id DESC";
                    ...
                    if($result = mysqli_query($link, $sql)){
                        if(mysqli_num_rows($result) > 0){
                            echo '<table class="table">';
                                echo "<thead>";
                                    echo "<tr>";
                                        echo "<th width='150'>N°</th>";
                                        echo "<th width='800'>Objectif</th>";
                                ...
                                while($row = mysqli_fetch_array($result)){
									...
									if($row['idom'] != '' OR $row['idom'] != 0) {
										echo '<td><b><a href="useromaccepted.php?id='. $row['id'] .'" class="me-3" >' . $row['idom'] . '</a></b></td>';
									} else {
										echo '<td><b><a href="useromaccepted.php?id='. $row['id'] .'" class="me-3" >OM-'. $year . '-' . $row['id'] . '</a></b></td>';
									}
                                        echo "<td>" . $row['obj'] . "</td>";
