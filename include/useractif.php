...
<div class="d-flex justify-content-between">
                        <h2 class="pull-left">Liste des utilisateurs actifs</h2>	
<h2 class="pull-right"><a href="adduser.php" class="btn btn-success"><i class="bi bi-plus"></i> Ajouter</a></h2>						
</div>
...
if($_SESSION['nom'] != 'Administrateur'){                    
    $sql = "SELECT id, nom, email, poste, depart, referto FROM usersom WHERE actif = 'Actif' AND nom != 'Administrateur' ORDER BY nom";
} else {
	$sql = "SELECT id, nom, email, poste, depart, referto FROM usersom WHERE actif = 'Actif' ORDER BY nom";
}
...
if ($result) {
    if (mysqli_num_rows($result) > 0) {
        ...
        echo '<div class="alert alert-danger"><em>Aucun utilisateur actif trouvé.</em></div>';
    }
} else {
